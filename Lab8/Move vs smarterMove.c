                ...
                for (int deltaRow = -1; deltaRow <= 1; deltaRow++)
                    for (int deltaCol = -1; deltaCol <= 1; deltaCol++, legalMove = false){
                        ...
                        if (legalMove){//change score from a function to a part of this code
                            tempScore = moveScore(boardCopy, cRow, cCol, turn, n);
                            if (tempScore > score){
                                score = tempScore;
                                *row = cRow; *col = cCol;
                            }
...
int makeSmarterMove (const char board[26][26], int n, char turn, int *row, int *col) {
    ...
    for (int cRow = n - 1; cRow >= 0; cRow--)//cRow is currentRow
        for (int cCol = n - 1; cCol >= 0; cCol--){//cCol is curretCol
...
int main(void) {
    ...
    int boardDimensions, *row, *col, compRow, compCol, *aiRow, *aiCol, AIRow, AICol;
    row = &compRow; col = &compCol; aiRow = &AIRow; aiCol = &AICol;
    ...
    while (availableMoves(gameBoard, boardDimensions, 'W') || availableMoves(gameBoard, boardDimensions, 'B')) {
        ...
        if (computer != turn) {
            if (availableMoves(gameBoard, boardDimensions, turn)) {
                makeSmarterMove(gameBoard, boardDimensions, turn, aiRow, aiCol);
                printf("Testing AI move (row, col): %c%c\n", *aiRow + 'a', *aiCol + 'a');
                validHumanMove = checkValidAndFlip(gameBoard, *aiRow, *aiCol, turn, boardDimensions);
